...
library. A test-only Mercury 22.01.8 C-backend consumer uses `pragma
foreign_proc("C", ...)` to invoke the same opaque ABI through the generated C
program and passed remotely in [Compiler Core #22](https://github.com/TangoSplicer/SynQ/actions/runs/31729572407); it is not a Mercury package, foreign type,
...
and the Clojure proof boundary is documented in [Clojure JNA Consumer Proof](./CLOJURE_JNA.md).
The Mercury proof boundary is documented in [Mercury C-Backend Consumer Proof](./MERCURY_C_ABI.md).

The expanded C consumer additionally parses one opaque in-memory program using
