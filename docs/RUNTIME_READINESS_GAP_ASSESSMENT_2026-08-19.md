...
Compiler Core run [#32270327206][1] verified the U6 implementation revision
`9a1a9b3` with 46/46 Linux recovery CTests, 32/32 Windows MSVC and macOS Clang
platform-neutral CTests, and three static-SDK consumer jobs. That evidence covers
bounded immutable and mutable classical evaluation, narrow U5/U6 local callable
frames, constrained quantum source expansion/simulation, and one exact U4
measurement-feedback pair.[2] [3]
...
| Evidence and interoperability | Six-job Compiler Core evidence, experimental C ABI fixtures, and a curated pinned source-fixture manifest. | The manifest is an entry index, not a complete conformance suite, stability policy, package ecosystem, or support commitment. |
...
| 5 | **Add reliability closure** | Run applicable sanitizers, parser/runtime fuzz or property tests, deterministic replay, and document known limits. | Not complete. Normal hardened builds and selected limits do not substitute for these controls. |
| 6 | **Create release closure** | Prove reproducible rebuild/install, define compatibility/deprecation policy, version examples and diagnostics, maintain a release checklist and known-limit ledger, then pass two consecutive unchanged six-job matrices. | Not complete. The build/install path is experimental; release and compatibility evidence are not yet closed. |
...
[1]: https://github.com/TangoSplicer/SynQ/actions/runs/32270327206 "Compiler Core #32270327206 — verified U6 binary callable implementation"
