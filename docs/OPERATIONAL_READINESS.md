...
| G2 — Executable semantic slice | **Remotely validated.** The bounded evaluators and local probability simulator have explicit opt-ins, resource limits, negative tests, one U4 terminal two-branch feedback model, and U5/U6 one- and two-formal local callable evaluation; the current 46-check Ubuntu recovery profile passed in [Compiler Core #32270327206](https://github.com/TangoSplicer/SynQ/actions/runs/32270327206). |
...
callable frame, U6 verifies one exactly-two-formal same-type local frame, and U4
verifies one feedback pair. Any wider callable or feedback model must first
specify result multiplicity/lifetime, `else`, scope, returns, target
representation, simulator semantics, error behavior, and tests together. None
of U4/U5/U6 is evidence of a general runtime.
