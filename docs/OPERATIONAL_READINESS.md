...
| G5 — Interoperability contract | **Remotely validated experimental foundation with a published experimental policy.** The versioned opaque C ABI and each advertised test consumer are independently exercised by the recovery profile, including [Compiler Core #45](https://github.com/TangoSplicer/SynQ/actions/runs/31887461976). [`EXPERIMENTAL_ABI_AND_DISTRIBUTION_POLICY.md`](./EXPERIMENTAL_ABI_AND_DISTRIBUTION_POLICY.md) defines the v1 change, ownership, and delivery boundary without promising stability. Installed packages, clean-install coverage, supported-platform evidence, and safe wrappers remain gaps. |
