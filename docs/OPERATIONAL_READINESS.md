...
| --- | --- | --- |
| Compiler foundation | [Compiler Core #32250265354](https://github.com/TangoSplicer/SynQ/actions/runs/32250265354) passed 44/44 Linux recovery-profile checks and 30/30 Windows/macOS platform-neutral checks for revision `62baf18`, plus Ubuntu 22.04, Windows MSVC, and macOS Clang static-SDK consumer jobs. | The grammar and semantics remain intentionally narrow. |
| Source model | Typed gates, bounded classical metadata/evaluation, qubit declarations with Alpha named operands, documented `if` source-lowering forms, bounded simulation, U3 one-gate routine templates, and U4 one-result/direct-`x` feedback exist behind exact gates/opt-ins. | No nested negation over results, binary-expression/alias/`else`/loop result control, scopes, returns, recursion, nested routine calls, routine execution, or general executable classical semantics exists. |
...
| G2 — Executable semantic slice | **Remotely validated.** The bounded evaluators and local probability simulator have explicit opt-ins, resource limits, negative tests, and one U4 terminal two-branch feedback model; the current 44-check Ubuntu recovery profile passed in [Compiler Core #32250265354](https://github.com/TangoSplicer/SynQ/actions/runs/32250265354). |
...
| G6 — Performance and reliability | **Remotely validated build/test configuration; local-only performance observation.** The warnings/hardening-enabled recovery compiler profile and its 44 tests passed in [Compiler Core #32250265354](https://github.com/TangoSplicer/SynQ/actions/runs/32250265354); the fixed benchmark timing is not a CI gate. Regression thresholds, sanitizers, and broader security review remain gaps. |
| G7 — Distribution and maintenance | **Remotely validated compiler baseline; locally validated delivery operations.** The versioned CLI and package configuration compile with the 44-check recovery profile in [Compiler Core #32250265354](https://github.com/TangoSplicer/SynQ/actions/runs/32250265354), while installation and CPack archive generation were run locally. Tags, signing, support policy, security disclosure route, and a maintained known-issues ledger remain gaps. |
...
The next buildable path must remain intentionally small: U4 has verified one
measurement result and one direct conditional correction, but any wider callable
or feedback model must first specify result multiplicity/lifetime, `else`, target
representation, simulator semantics, error behavior, and tests together. U4
source lowering and branch enumeration are not evidence of a general runtime.
