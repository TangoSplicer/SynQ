...
[Compiler Core platform-matrix run #32394772652](https://github.com/TangoSplicer/SynQ/actions/runs/32394772652)
passed all seven independent jobs for revision `fdbbbec`: the ordinary Linux
recovery profile passed **48/48** CTest checks; Windows MSVC and macOS Clang each
passed **34/34** platform-neutral CTest checks; Ubuntu 22.04, Windows MSVC, and
macOS Clang each passed the experimental static-SDK consumer; and an additive
Linux/Clang ASan/UBSan profile passed **33/33** isolated core tests. The exact
...
| --- | --- | --- |
| Compiler core | **Remotely validated, independently scoped profiles.** | The Ubuntu Linux profile builds `libsynq_lib.a`, `synqc`, test-only `libsynq_ffi.so`, and passes 48 CTests. Separate Windows MSVC and macOS Clang profiles each pass 34 platform-neutral compiler/CLI/C-ABI CTests. An additive Linux/Clang 33-test ASan/UBSan profile covers only its isolated core configuration. Optional historical targets remain outside these claims. |
...
| Non-publishing `synqc` candidate archive | **Remotely validated CI candidate path in [synqc Release Candidate #32393670587](https://github.com/TangoSplicer/SynQ/actions/runs/32393670587).** | Linux, macOS ARM64, and Windows X64 jobs built an exact CLI-only archive, JSON/SHA-256 manifests, content allowlist, clean extraction, `--version`/`--help`/SynQ validation smoke, GitHub provenance attestation, and retained workflow artifact. The candidate files are not GitHub Release assets, public downloads, signed binaries, notarized software, installers, or package-manager packages. |
