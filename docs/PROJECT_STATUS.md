...
| Compiler smoke suite | **Passes in the recovery profile** | `ctest --test-dir <build> --output-on-failure` completed with 3/3 focused smoke tests passing. They cover binary-format round-trip and malformed-input bounds handling; a bounded line-oriented recovery parser with `let <identifier> = <value>` declarations and whitespace-delimited `//` comments; rejection of malformed, unsupported, and missing input; URL preservation; runtime placeholder paths; and a strict OpenQASM 3 source-export subset. The parser grammar is intentionally narrow and is not a complete SynQ language implementation. |
| OpenQASM 3 source export | **Passes locally for a bounded subset** | `synq_openqasm3_exporter_smoke` verifies exact OpenQASM 3 output for `quantum h`, `x`, `y`, `z`, and `bell_pair`, plus parser-to-exporter ordering and diagnostic rejection of all declarations, classical instructions, AI instructions, and unsupported quantum kernels. It does not import, execute, externally parse, or submit OpenQASM. |
