...
| Compiler smoke suite | **Passes in the recovery profile** | `ctest --test-dir <build> --output-on-failure` completed with 4/4 focused smoke tests passing. They cover binary-format round-trip and malformed-input bounds handling; a bounded line-oriented recovery parser with declarations, whitespace-delimited `//` comments, and explicit `q[index]` operands for quantum instructions; runtime placeholder paths; strict OpenQASM 3 export; and reference-parser acceptance of the generated OpenQASM fixture. The parser grammar is intentionally narrow and is not a complete SynQ language implementation. |
| OpenQASM 3 source export | **Passes locally for a bounded subset with reference-parser validation** | `synq_openqasm3_exporter_smoke` verifies exact OpenQASM 3 output for `quantum h`, `x`, `y`, `z`, `cx`, and `bell_pair`, including explicit qubit operands, allocation, and diagnostic rejection of unsupported statements or malformed operands. `synq_openqasm3_reference_parse` validates the generated output with pinned `openqasm3[parser]==1.0.1`. This does not import OpenQASM, establish semantic correctness, execute, or submit to a provider. |
...
| Compiler-core CI workflow | **Passes on GitHub Actions** | `.github/workflows/compiler-core.yml` configures Ubuntu-based build and smoke-test steps for the verified recovery profile. [Compiler Core #5](https://github.com/TangoSplicer/SynQ/actions/runs/31707219450) completed successfully in 44 seconds for revision `f570cc9`, including the bounded OpenQASM 3 exporter fixture. This does not validate optional compiler targets. |
