...
[Compiler Core platform-matrix run #32374149046](https://github.com/TangoSplicer/SynQ/actions/runs/32374149046)
passed all seven independent jobs for revision `8fc1de5`: the ordinary Linux
recovery profile passed **47/47** CTest checks; Windows MSVC and macOS Clang each
passed **33/33** platform-neutral CTest checks; Ubuntu 22.04, Windows MSVC, and
...
| --- | --- | --- |
| Compiler core | **Remotely validated, independently scoped profiles.** | The Ubuntu Linux profile builds `libsynq_lib.a`, `synqc`, test-only `libsynq_ffi.so`, and passes 47 CTests. Separate Windows MSVC and macOS Clang profiles each pass 33 platform-neutral compiler/CLI/C-ABI CTests. An additive Linux/Clang 33-test ASan/UBSan profile covers only its isolated core configuration. Optional historical targets remain outside these claims. |
