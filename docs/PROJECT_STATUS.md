...
## Current verified baseline
 ...
 [Compiler Core platform-matrix run #32075281245](https://github.com/TangoSplicer/SynQ/actions/runs/32075281245)
passed all five independent jobs for revision `5558811`: the Linux recovery
profile passed **33/33** CTest checks; Windows MSVC and macOS Clang each passed
**24/24** platform-neutral CTest checks; and Ubuntu 22.04 plus Windows MSVC each
...
| Bounded if export | **Remotely validated strict source-lowering subset in [Compiler Core #32075281245](https://github.com/TangoSplicer/SynQ/actions/runs/32075281245).** | One literal `if`, one `if` controlled by an earlier top-level Boolean-literal declaration, or `if not <that declaration>`, with one supported typed gate body lowers to OpenQASM 3. Boolean literal negation, binary expressions, aliases, measurement results, `while`, measurement bodies, and execution remain rejected. |
...
1. Define target-side Boolean literal-negation folding, binary-expression
   evaluation, writes, lifetime, and measurement-result storage before accepting
   wider Boolean expressions, aliases, loops, branch blocks, or measurement bodies.
