...
## Current verified baseline
 ...
 [Compiler Core platform-matrix run #32247154982](https://github.com/TangoSplicer/SynQ/actions/runs/32247154982)
passed all six independent jobs for revision `be18e0f`: the Linux recovery
profile passed **41/41** CTest checks; Windows MSVC and macOS Clang each passed
**28/28** platform-neutral CTest checks; and Ubuntu 22.04, Windows MSVC, and macOS
...
| Typed source model | **Remotely validated bounded subset.** | Supports documented immutable declarations, Alpha top-level mutable `var`/`set` cells, typed gates, unnamed/named measurement metadata, Alpha expressions, declaration-only functions, zero-parameter kernels, and U3 parameterized kernels over literal angles and explicit qubit operands. No returns, local scopes, recursion, nested routine calls, branch execution, general expressions, target-side state lowering, or general runtime semantics exist. |
...
1. Define measurement-result storage and target-side feedback only after local
