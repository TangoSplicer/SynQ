...
passed all six independent jobs for revision `7307c00`: the Linux recovery
profile passed **45/45** CTest checks; Windows MSVC and macOS Clang each passed
**31/31** platform-neutral CTest checks; and Ubuntu 22.04, Windows MSVC, and macOS
...
| --- | --- | --- |
| Compiler core | **Remotely validated, independently scoped profiles.** | The Ubuntu Linux profile builds `libsynq_lib.a`, `synqc`, test-only `libsynq_ffi.so`, and passes 45 CTests. Separate Windows MSVC and macOS Clang profiles each pass 31 platform-neutral compiler/CLI/C-ABI CTests. Optional historical targets remain outside these claims. |
| `synqc` CLI | **Remotely validated experimental workflow.** | Supports validation, bounded AST/strict-Hybrid OpenQASM source output, immutable/state evaluation, U5 `--eval-runtime`, and bounded local probabilities. It is not a general executor, provider client, REPL, package manager, or stable CLI. |
| Typed source model | **Remotely validated bounded subset.** | Supports immutable declarations, Alpha top-level mutable cells, typed gates, named measurement provenance, Alpha expressions, zero-parameter/U3 parameterized kernels, one U4 feedback pair, and U5 one-formal local functions. No general returns, lexical scopes, recursion, nested routine calls, general branch execution, target-side state lowering, or general runtime semantics exist. |
...
1. Define wider callable and control contracts only after parameters, scopes, returns,
