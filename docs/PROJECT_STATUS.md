...
[Compiler Core platform-matrix run #32403435066](https://github.com/TangoSplicer/SynQ/actions/runs/32403435066)
passed all seven independent jobs for revision `8125287`: the ordinary Linux
...
| Non-publishing `synqc` candidate archive | **Remotely validated tag-aware CI candidate path in [synqc Release Candidate #32401780347](https://github.com/TangoSplicer/SynQ/actions/runs/32401780347).** | Linux, macOS ARM64, and Windows X64 jobs built an exact CLI-only archive, JSON/SHA-256 manifests, content allowlist, clean extraction, `--version`/`--help`/SynQ validation smoke, GitHub provenance attestation, and retained workflow artifact. An optional existing-tag input rejects malformed, reserved, and tag-to-checkout mismatch cases before embedding a tag in the manifest. The current run used `not-a-release`; candidate files are not GitHub Release assets, public downloads, signed binaries, notarized software, installers, or package-manager packages. |
