...
| `LCEP-COMP-002` | Existing immutable Integer/Boolean/String values, typed mutable cells, and U5/U6 typed callable results retain their documented evaluator-mode acceptance and rejection boundaries. | Positive typed-value and negative mode-separation smoke. | `synq_local_classical_evaluation_value_composition_smoke` | Intended platform-neutral core profile. | Local recovery 50/50 and isolated sanitizer 35/35 passed; remote matrix pending |
