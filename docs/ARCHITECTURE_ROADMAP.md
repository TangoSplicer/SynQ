...
| `evaluate_bounded_constants` | Locally evaluates an explicitly opted-in, declaration-only resolved subset of Integer/Boolean/String literals, aliases, and one-operator checked Integer arithmetic with a caller-set declaration limit. | It rejects quantum statements, measurements, controls, callables, decimals, opaque source, I/O, mutation, loops, calls, simulation, and hardware use. |
| Core CMake profile | Builds C++17 library sources and registers focused smoke tests by default. | The default options keep historical optional targets off; [Compiler Core #39](https://github.com/TangoSplicer/SynQ/actions/runs/31849507058) validated the current 23-test profile, including declaration-only callable provenance/rejection/resolver-preservation coverage, external OpenQASM validation, and C ABI consumers. |
