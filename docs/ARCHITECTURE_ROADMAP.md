...
| `export_openqasm3` | Lowers typed AST quantum nodes to the supported gates (`h`, `x`, `y`, `z`, `cx`, `bell_pair`, `rx`, `ry`, `rz`, `p`) and unnamed typed measurements to matching indexed OpenQASM classical bits. `export_hybrid_openqasm3` locally validates a strict typed Hybrid IR subset with one explicit `qubit q[n]` declaration. | A temporary adapter supports direct legacy instruction fixtures. The Hybrid path rejects classical declarations, named result declarations, controls, named registers, missing declarations, and range violations. Export is source generation, not execution.[4] |
| Experimental feature registry | Registers named alpha/beta/stable feature gates; `parameterized-quantum-gates`, `classical-control-flow`, `integer-arithmetic-expressions`, `qubit-declarations`, and `callable-declarations` are remotely validated Alpha gates. | The recovery parser recognizes an exact file-scoped annotation. The arithmetic gate promotes only annotated one-operator Integer expressions; the qubit gate permits one positive-size named source declaration; the callable gate permits declaration-only `fn`/`kernel` names. Structured warning output and removal records remain future work. |
...
| `simulate_bounded_quantum` | Locally calculates pure-state basis and requested marginal probabilities from one explicit default register, supported gates, and unnamed trailing measurements. | It has explicit opt-in, 10-qubit/1,024-gate default limits, no sampling/collapse/noise, and rejects all controls, named results, provider access, and hardware use. |
| `synqc` recovery CLI | Remotely exposes parser/lowering/resolution validation, bounded AST OpenQASM 3 source emission, and explicit declaration-only constant evaluation as separate modes. | It is a new supported recovery-profile executable, not the historical runtime-dependent CLI. It has no general execution, simulation, provider, package, REPL, or stable-release contract. |
| Core CMake profile | Builds C++17 library sources and registers focused smoke tests by default. | The default options keep historical optional targets off; [Compiler Core #42](https://github.com/TangoSplicer/SynQ/actions/runs/31886580043) validated the current 25-test profile, including the separate-process CLI workflow, bounded constant evaluation, declaration-only callable provenance/rejection/resolver-preservation coverage, external OpenQASM validation, and C ABI consumers. |
