...
| --- | --- | --- |
| `Parser::parseFile` | Reads files line by line; handles selected declarations, instructions, comments, explicit operands, literal angles, Alpha Boolean conditions, one Alpha integer arithmetic declaration operator, and one Alpha positive-size qubit declaration. | The grammar is intentionally small and does not implement general expressions, scopes, functions, or execution.[2] |
| `ProgramNode`, `InstructionNode`, `DeclarationNode`, `QubitDeclarationNode`, `QuantumGateNode`, `MeasurementNode` | Retain a program as a list of AST-node pointers; parsed quantum statements use a typed gate kind, source name, optional literal angle, numeric operands, and source line; parsed declarations retain source text plus a non-evaluating literal or Alpha arithmetic classification; qubit declarations retain a named positive source size; measurements retain one qubit index and provenance. | Function/class nodes, recursive expression syntax, runtime qubit allocation, operand-bound validation, named result storage, ownership, and a general typed AST remain future work.[3] |
...
| Core CMake profile | Builds C++17 library sources and registers focused smoke tests by default. | The default options keep historical optional targets off; [Compiler Core #31](https://github.com/TangoSplicer/SynQ/actions/runs/31847601825) validated the preceding 19-test profile. The locally validated 20-test profile adds the bounded qubit-declaration smoke before the existing external OpenQASM validation and C ABI consumers. |
