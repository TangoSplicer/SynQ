...
| `synqc` recovery CLI | Locally exposes parser/lowering/resolution validation, bounded AST OpenQASM 3 source emission, and explicit declaration-only constant evaluation as separate modes. | It is a new supported recovery-profile executable, not the historical runtime-dependent CLI. It has no general execution, simulation, provider, package, REPL, or stable-release contract. |
| Core CMake profile | Builds C++17 library sources and registers focused smoke tests by default. | The default options keep historical optional targets off; [Compiler Core #41](https://github.com/TangoSplicer/SynQ/actions/runs/31886373263) validated the current 24-test profile, including bounded constant evaluation, declaration-only callable provenance/rejection/resolver-preservation coverage, external OpenQASM validation, and C ABI consumers. |
