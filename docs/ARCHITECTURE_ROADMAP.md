...
| `synqc` recovery CLI | Remotely exposes parser/lowering/resolution validation, bounded AST OpenQASM 3 source emission, declaration-only constant evaluation, and bounded local probability simulation; its locally validated strict Hybrid export mode preserves explicit-register constraints. | It is a new supported recovery-profile executable, not the historical runtime-dependent CLI. It has no general execution, device simulation, provider, package, REPL, or stable-release contract. |
