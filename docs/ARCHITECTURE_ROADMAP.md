...
| C ABI foundation | An opaque-handle C header parses recovery-profile files and exports bounded OpenQASM 3 text. | The experimental ABI has a compiled C smoke consumer, version identifier, and published experimental change/distribution policy, but is not frozen or distributed as an SDK. See [`C_ABI.md`](./C_ABI.md) and [`EXPERIMENTAL_ABI_AND_DISTRIBUTION_POLICY.md`](./EXPERIMENTAL_ABI_AND_DISTRIBUTION_POLICY.md). |
