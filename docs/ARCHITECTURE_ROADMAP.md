...
| `ProgramNode`, `InstructionNode`, `DeclarationNode`, `QuantumGateNode` | Retain a program as a list of AST-node pointers; parsed quantum statements now use a typed gate kind, source name, optional literal angle, numeric operands, and source line. | Function/class nodes, classical types, measurement, ownership, and a general typed AST remain future work.[3] |
