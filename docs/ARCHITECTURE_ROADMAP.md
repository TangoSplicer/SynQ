...
| --- | --- | --- |
| `Parser::parseFile` | Reads files line by line; handles selected declarations, instructions, comments, explicit operands, literal angles, Alpha Boolean conditions, and one Alpha integer arithmetic declaration operator. | The grammar is intentionally small and does not implement general expressions, scopes, functions, or execution.[2] |
| `ProgramNode`, `InstructionNode`, `DeclarationNode`, `QuantumGateNode`, `MeasurementNode` | Retain a program as a list of AST-node pointers; parsed quantum statements use a typed gate kind, source name, optional literal angle, numeric operands, and source line; parsed declarations retain source text plus a non-evaluating literal or Alpha arithmetic classification; measurements retain one qubit index and provenance. | Function/class nodes, recursive expression syntax, named result storage, ownership, and a general typed AST remain future work.[3] |
...
| Experimental feature registry | Registers named alpha/beta/stable feature gates; `parameterized-quantum-gates`, `classical-control-flow`, and `integer-arithmetic-expressions` are Alpha gates. | The recovery parser recognizes an exact file-scoped annotation. The arithmetic gate promotes only annotated one-operator Integer expressions; unannotated expression-like text remains opaque for compatibility. Structured warning output and removal records remain future work. |
...
| Core CMake profile | Builds C++17 library sources and registers focused smoke tests by default. | The default options keep historical optional targets off; the current locally validated profile contains 19 smoke/interop checks, including the bounded arithmetic resolver test, external OpenQASM validation, and C ABI consumers. Remote evidence through Compiler Core #30 covers the preceding 18-test profile only. |
