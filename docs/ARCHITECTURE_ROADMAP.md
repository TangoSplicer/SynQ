...
| `ProgramNode`, `InstructionNode`, `DeclarationNode` | Retain a program as a list of AST-node pointers with source-line fields on supported node types. | The AST itself labels future nodes such as functions and classes as future work.[3] |
| `export_openqasm3` | Emits a header and supported gates (`h`, `x`, `y`, `z`, `cx`, `bell_pair`, `rx`, `ry`, `rz`, `p`) only when constraints are satisfied. | Unsupported statements/gates yield diagnostics. Export is source generation, not execution.[4] |
