...
| Compiler smoke suite | `ctest --test-dir /tmp/synq-parameterized-compatibility --output-on-failure` | Passed: 5/5 `synq_core_smoke`, `synq_parser_runtime_smoke`, `synq_openqasm3_exporter_smoke`, `synq_openqasm3_reference_parse`, and `synq_openqasm3_qiskit_import` | The parser fixture covers only a deliberately bounded recovery grammar with explicit qubits and literal-angle parameters; the exporter checks fixed and parameterized mappings, operand and arity rejection; the generated fixture is accepted by pinned `openqasm3[parser]==1.0.1` and converted by `qiskit-qasm3-import==0.6.0`; no provider or hardware is involved |
