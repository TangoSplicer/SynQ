...
| Compiler smoke suite | `ctest --test-dir /tmp/synq-expanded-compatibility --output-on-failure` | Passed: 4/4 `synq_core_smoke`, `synq_parser_runtime_smoke`, `synq_openqasm3_exporter_smoke`, and `synq_openqasm3_reference_parse` | The parser fixture covers only a deliberately bounded line-oriented recovery grammar with explicit qubit operands; the exporter checks fixed gate mappings, operand and arity rejection; the generated fixture is accepted by pinned `openqasm3[parser]==1.0.1`; no SDK or hardware is involved |
