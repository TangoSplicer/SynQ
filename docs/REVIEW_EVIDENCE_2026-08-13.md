...
| Compiler smoke suite | `ctest --test-dir /tmp/synq-three-improvement-check --output-on-failure` | Passed: 2/2 `synq_core_smoke` and `synq_parser_runtime_smoke` | The parser fixture covers only a deliberately bounded line-oriented recovery grammar, including one declaration form; runtime checks cover placeholder paths, not external execution |
...
| Repository frontend build | `pnpm run build` in `frontend/` | Completed successfully | Lazy route/example boundaries and vendor chunks removed Vite's former 500 kB chunk warning; browser review confirmed the factual recovery-status page and local-only editor notice render |
