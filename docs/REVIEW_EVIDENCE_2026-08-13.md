...
| Compiler smoke suite | `ctest --test-dir /tmp/synq-continued-recovery --output-on-failure` | Passed: 2/2 `synq_core_smoke` and `synq_parser_runtime_smoke` | The parser fixture covers only a deliberately bounded line-oriented recovery grammar, including one declaration form and whitespace-delimited comments; runtime checks cover placeholder paths, not external execution |
