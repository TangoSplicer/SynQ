...
| --- | --- | --- | --- |
| **P0** | Versioned ABI and distribution policy | The C ABI is experimentally exercised but is neither stable nor distributable as a supported SDK. | A published compatibility/ownership/deprecation policy, ABI conformance fixtures, installable artifact strategy, and CI matrix without claiming stable ABI prematurely. |
