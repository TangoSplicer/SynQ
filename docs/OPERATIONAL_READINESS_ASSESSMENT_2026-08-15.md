...
| --- | --- | --- | --- |
| **P0** | Versioned ABI and distribution policy | The experimental compatibility, ownership, and distribution boundary is now published in [`EXPERIMENTAL_ABI_AND_DISTRIBUTION_POLICY.md`](./EXPERIMENTAL_ABI_AND_DISTRIBUTION_POLICY.md); the C ABI remains neither stable nor distributable as a supported SDK. | ABI conformance matrix, installed native SDK artifact strategy, clean installation tests, and a supported-platform matrix without claiming stable ABI prematurely. |
