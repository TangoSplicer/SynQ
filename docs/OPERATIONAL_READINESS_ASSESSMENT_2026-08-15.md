...
| --- | --- | --- | --- |
| **P0** | Versioned ABI and distribution policy | The experimental compatibility, ownership, and distribution boundary is published in [`EXPERIMENTAL_ABI_AND_DISTRIBUTION_POLICY.md`](./EXPERIMENTAL_ABI_AND_DISTRIBUTION_POLICY.md), and a static native SDK clean-install path has local evidence in [`EXPERIMENTAL_NATIVE_SDK.md`](./EXPERIMENTAL_NATIVE_SDK.md). The C ABI remains neither stable nor distributable as a supported SDK. | Remote SDK CI evidence, ABI conformance matrix, artifact-version policy, signing/provenance, and a supported-platform matrix without claiming stable ABI prematurely. |
