...
quantum subset. The current recovery profile passed **46/46** CTest checks
...
and bounded parameterized quantum routines with explicit qubit/angle formals,
...
two-branch probability enumeration, plus U5 one-formal and U6 two-formal
same-type local classical callable evaluation with explicit source/export/
simulator/ABI rejection boundaries.
They do **not** make
...
> increments. U5 verifies one exact local callable frame, U6 one exact
> two-formal same-type local frame, and U4 one feedback pair only. General
> target-side classical storage, `else`, wider branch execution, scopes, and
> general calls remain separate later contracts.
...
| --- | --- | --- |
| Build and test | Build the default C++17 recovery profile and run 46 focused CTest checks on the Ubuntu full profile. Independent Windows/MSVC and macOS/Clang profiles each run 32 platform-neutral checks.[1] | Historical optional targets remain outside this profile, and the two non-Linux counts do not establish a support commitment.[3] |
...
| Quantum source model | Use a small typed gate subset: `h`, `x`, `y`, `z`, `cx`, `bell_pair`, and literal-angle `rx`, `ry`, `rz`, or `p`, bounded typed measurement syntax, Alpha named-register operands, U3 one-gate routines, and U4 one-result/direct-`x` feedback. | No general gate language, dynamic indexing, lifetime model, routine capture/allocation/execution, aliasing, deallocation, dynamic allocation, or hardware semantics exists.[2] [5] |
| Classical source model | Use literal declarations, earlier aliases, exact Alpha-gated one-operator Integer arithmetic and Boolean forms, feature-gated top-level `var`/`set` cells with fixed static types, U5 one-formal local calls, and U6 same-type two-formal local calls. `--inspect-semantics` reports binding/cell metadata without evaluation.[4] | No general expressions, nested scopes, decimal evaluation, arbitrary calls, runtime values, target-side state, or executable classical control flow exists. |
| OpenQASM output | Export a bounded AST subset; strict Hybrid export supports documented literal/immutable-Boolean `if` forms and the exact U4 named-measurement/direct-`x` form, with reference-parser validation. | Export is source generation only; it does not execute circuits, prove semantic equivalence, lower binary/alias controls, or support general named-result control. |
| Local quantum calculation | Calculate deterministic pure-state basis and marginal probability data across explicit default or named registers with declaration-order physical allocation and fixed combined limits.[5] U4 adds one terminal two-branch measurement/direct-`x` model without a sampled host value. | No general sampling/collapse API, noise, general control-flow execution, provider connection, hardware submission, resource lifecycle, or dynamic allocation occurs. |
| Classical evaluation | Evaluate opt-in immutable Integer/Boolean/String literals, aliases, checked bounded expressions, separate top-level mutable Boolean/Integer/String cells, U5 one-formal calls, and U6 two-formal same-type calls in source order. | This is not a general interpreter, target runtime, or branch executor. |
...
| G2 — Executable semantic slice | Complete for bounded immutable evaluation, bounded local mutable state, U5/U6 local callable evaluation, and bounded local probability calculation. | Remotely validated. | Design general execution separately; do not extrapolate from the bounded evaluators/simulator. |
...
| **P1** | Wider callable bodies and calls | U5/U6 define non-capturing one- and same-type two-formal local calls only. Arbitrary arity, mixed types, scopes, returns, recursion, resource effects, and quantum-resource calls remain semantic decisions. | A separately contracted bounded next subset, call/resource rules, diagnostics, local/target rejection behavior, and conformance fixtures. |
...
### Stage B — Bounded measurement feedback — completed Alpha slice

The literal-`if`, compile-time literal-negation-`if`, immutable Boolean-
identifier forms, and U4 one-result/direct-`x` form are complete. U4 declares
its target scalar bit, result lifetime, immediate correction, branch-enumeration
model, ABI rejection, and source-only boundary together. Before accepting nested
negation, aliases, binary conditions, `else`, loops, branch blocks, multiple
results, or measurement bodies, SynQ must specify their representation and
separate target lowering from local execution.
...
### Stage C — U5/U6 local callable bodies and calls — completed bounded slice; wider callable/control design next

U5 admits one non-capturing local formal and U6 admits exactly two ordered
same-type non-capturing local formals, but neither admits general callable bodies
or calls. Only after feedback, scopes, resource effects, and the chosen control
model have a stable minimal design should SynQ widen callable behavior. A safe
next design must still exclude recursion, dynamic dispatch, unbounded allocation,
implicit capture, and uncontrolled effects while specifying visibility,
arguments, return rules, quantum resource passing, and lowering or execution
support per backend.

### Stage D — Platform and delivery evidence maintenance

The macOS Clang smoke and clean-install static-SDK consumer profiles now both
pass. Continue to keep the separately scoped platform evidence visible: it does
not establish a stable ABI, shared-library delivery, package ecosystem, or
general cross-platform packaging claim.[1] [3]
...
The recommended immediate scope is now a **wider callable/control design
contract**, not implementation breadth. This preserves remotely verified U3
resource/source expansion, U4 measurement feedback, and U5/U6 local callable
frames alongside the completed semantic-inspection, multi-register-simulation,
source-only Rust-wrapper, strict `if` lowering, and local-only mutable-state work
while avoiding unsupported general target-side state or branch-execution surface
area. It provides prospective contributors and adopters a clearer, reproducible
contract than unverified breadth and is more credible for community growth,
revenue exploration, and future acquisition discussions.
