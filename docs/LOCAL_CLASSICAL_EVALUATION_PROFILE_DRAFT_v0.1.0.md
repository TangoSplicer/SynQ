...
| `LCEP-COMP-001` | Narrow fixtures cover shared immutable declarations, state/callable crossings, typed value diversity, and direct quantum crossings; `LCEP-COMP-001` and `LCEP-COMP-002` are remotely verified. `LCEP-COMP-003` now covers the remaining parser-resolvable quantum/control dispatch categories and passed local recovery (51/51) plus isolated sanitizer (36/36), but is not yet remotely verified. | Verify `LCEP-COMP-003`, then evaluate only the separately documented grammar/effect, limit, regression, and diagnostic closure gaps. |
