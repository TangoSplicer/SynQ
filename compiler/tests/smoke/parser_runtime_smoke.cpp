...
bool parser_accepts_supported_fixture() {
    ...
    if (!require(program->statements.size() == 4, "parser retains all supported instructions")) return false;

    const std::string expected_operations[] = {"print", "delay", "quantum", "ai"};
    const std::string expected_arguments[] = {"hello from fixture", "0", "bell_pair", "concise_summary"};
    for (std::size_t index = 0; index < program->statements.size(); ++index) {
        ...
        if (!require(instruction->line == index + 2, "parser records source line")) return false;
    }
    return true;
}
