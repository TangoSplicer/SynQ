...
bool parser_reports_structured_recovery_errors() {
    ...
    const std::string semantic_path = write_fixture("synq_diagnostic_semantic_gate.synq", "quantum cx q[0]\n");
    const std::string duplicate_path = write_fixture("synq_diagnostic_duplicate_declaration.synq", "let theta = 0.5\nlet theta = 1.0\n");
    const std::string measurement_path = write_fixture("synq_diagnostic_measurement.synq", "measure q[0], q[1]\n");
    Parser parser;
    ...
    const auto semantic_gate = parser.parseFileWithDiagnostics(semantic_path);
    const auto duplicate = parser.parseFileWithDiagnostics(duplicate_path);
    const auto measurement = parser.parseFileWithDiagnostics(measurement_path);
    ...
    std::remove(semantic_path.c_str());
    std::remove(duplicate_path.c_str());
    std::remove(measurement_path.c_str());
    ...
    return require(has_single_error(declaration, "SYNQ-P002", 1), "malformed declarations receive SYNQ-P002") &&
           ...
           require(has_single_error(semantic_gate, "SYNQ-S002", 1), "malformed known gate shapes receive SYNQ-S002") &&
           require(has_single_error(duplicate, "SYNQ-S004", 2), "duplicate declarations receive SYNQ-S004 at the later binding") &&
           require(has_single_error(measurement, "SYNQ-P008", 1), "malformed measurements receive SYNQ-P008");
}
