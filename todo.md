...
- [x] Select a non-colliding experimental candidate tag and prepare an exact tag-to-commit-to-archive-hash manifest without creating a public GitHub Release (`v0.1.0-experimental` proposed for candidate commit `fdbbbec43b75ec7cbc2efef1c62ec951f5800230`; tag remains uncreated).
...
- [x] Implement and remotely verify a tag-aware non-publishing candidate workflow that accepts a validated tag input, rejects a tag-to-checkout mismatch, embeds the confirmed tag in the manifest, and retains the no-release-permission boundary (synqc Release Candidate #32401780347; exact-revision Compiler Core #32403435066; tag remains uncreated).
- [ ] After explicit user confirmation, publish only a candidate whose exact tag, archive hashes, clean-extraction evidence, provenance result, and full required matrix are recorded in the release notes.
