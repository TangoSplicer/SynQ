...
- [x] Publish a versioned public conformance-fixture manifest with direct repository download links, source suite ownership, verification run provenance, and explicit scope boundaries.
...
- [x] Publish a narrowly defined complete-runtime claim standard that distinguishes a bounded SynQ runtime from a hardware service, stable ABI, or general-purpose language claim.
- [x] Specify, implement, locally validate, remotely verify, and document the U5 bounded one-formal local classical callable evaluator before claiming any callable execution (Compiler Core #32266056516: 45/45 Linux, 31/31 Windows MSVC and macOS Clang, plus three clean-install SDK jobs).
- [x] Specify, implement, locally validate, remotely verify, and document the U6 bounded two-formal same-type local classical callable evaluator before claiming binary callable execution (Compiler Core #32270327206: 46/46 Linux, 32/32 Windows MSVC and macOS Clang, plus three clean-install SDK jobs).
- [x] Maintain a current evidence-based runtime-readiness gap assessment that distinguishes verified bounded subsets from the remaining complete-runtime acceptance gates.
- [ ] Make zero-cost operation an acceptance gate: every runtime build, test, example, release artifact, and documentation path must use repository tooling, local execution, or free CI only, with no paid provider dependency.
- [ ] Make beginner learnability an acceptance gate: each executable subset feature must have one plain-language tutorial, one minimal runnable example, one safe failure example, and diagnostic wording reviewed for first-time users.
- [ ] Make differentiated hybrid design an acceptance gate: each increment must state its distinctive provenance/resource/feedback guarantee and compare it against an existing general-purpose or hardware-service pattern without fabricating superiority claims.
- [ ] Make security evidence an acceptance gate: every runtime feature must define input/operation/depth/memory limits, deterministic failure behavior, opt-in gates, ABI rejection or ownership behavior, and applicable sanitizer/fuzz coverage.
- [ ] Specify and implement a coherent runtime value model, lexical scopes, typed bindings, deterministic evaluation order, and structured runtime-error behavior with explicit resource limits.
...
- [x] Publish, implement, locally validate, and remotely verify a deterministic bounded parser/runtime fuzz-replay target with a checked-in seed/corpus contract and explicit failure-reproduction command (Compiler Core #32374149046: 47/47 ordinary Linux, 33/33 Windows/macOS, three SDK consumers, 33/33 sanitizer core CTests).
...
- [x] Implement and remotely verify a non-publishing Linux/macOS/Windows synqc release-candidate workflow that produces CLI-only archives, SHA-256 and JSON manifests, content-allowlist checks, and clean-extraction smoke evidence for the exact source revision (synqc Release Candidate #32393670587; exact-revision Compiler Core #32394772652).
- [x] Add and remotely verify optional GitHub provenance attestations for final-form candidate archives, documenting verification without representing provenance as a security certification (synqc Release Candidate #32393670587).
- [x] Select an experimental candidate tag and prepare an exact tag-to-commit-to-archive-hash manifest (`v0.1.0-experimental` published at candidate commit `812528711dc9189ba7d1af27657821b9d54a39ab`; final archive record and release link are published).
- [x] Publish a user-facing archive-verification guide and reusable experimental release-notes template that preserve the unsigned/notarized and non-package-manager boundaries.
- [x] Implement and remotely verify a tag-aware candidate workflow that accepts a validated tag input, rejects a tag-to-checkout mismatch, embeds the confirmed tag in the manifest, and retains the no-release-permission boundary (final tagged synqc Release Candidate #32482750179; exact-revision Compiler Core #32403435066; public pre-release published only after separate confirmation).
- [x] After explicit user confirmation, publish only a candidate whose exact tag, archive hashes, clean-extraction evidence, provenance result, and full required matrix are recorded in the release notes (`v0.1.0-experimental`; release link and final record published).
- [x] Draft a named local classical evaluation profile that records the existing constants, mutable-state, and U5/U6 callable modes, value model, scope, evaluator separation, budgets, diagnostic categories, and non-claims without asserting runtime completion.
- [x] Implement `LCEP-COMP-001`: add a narrow profile-level positive/negative composition fixture for existing local constants, state, and callable evaluator modes without expanding language semantics; full local recovery (49/49) and isolated sanitizer (34/34) passed.
- [x] Remotely verify `LCEP-COMP-001` in the required seven-job Compiler Core matrix before calling its mode-separation evidence verified or changing public capability claims (Compiler Core #32485055898: 49/49 Linux, 35/35 Windows/macOS, three SDK consumers, 34/34 sanitizer CTests).
- [x] Specify and implement `LCEP-COMP-002`: verify cross-mode agreement for supported immutable Integer, Boolean, and String declarations, plus existing typed state and U5/U6 runtime success/rejection behavior, without creating any mixed-mode evaluator semantics; local recovery (50/50) and isolated sanitizer (35/35) passed.
- [x] Remotely verify `LCEP-COMP-002` in the required seven-job Compiler Core matrix before calling its value-diversity evidence verified or changing public capability claims (Compiler Core #32496656921: 50/50 Linux, 36/36 Windows/macOS, three SDK consumers, 35/35 sanitizer CTests).
- [x] Specify and implement `LCEP-COMP-003`: exhaustively cover the current local-evaluator top-level matrix for immutable declarations, mutable declarations, assignments, U5/U6 callable declarations and invocations, parameterized quantum routine declaration/call sources, qubit declarations, gates, measurements, and controls; assert each mode’s documented acceptance or atomic rejection without adding a mixed-mode evaluator; local recovery (51/51) and isolated sanitizer (36/36) passed.
- [x] Remotely verify `LCEP-COMP-003` in the required seven-job Compiler Core matrix before calling the current dispatch-category matrix complete or changing public capability claims (Compiler Core #32533581417: 51/51 Linux, 37/37 Windows/macOS, three SDK consumers, 36/36 sanitizer CTests).
